...
|---|---|---|---|
| VS Code (`engines.vscode`) | `^1.100.0` | `1.136.1`（2026-09-03 发布） | 保持 `^1.100.0`（扩展未使用新 API） |
| `@types/vscode` | `1.100.0` | `1.136.0` | 锁定 `1.100.0`，与引擎对齐 |
| TypeScript | `^5.9.2` | — | 可用 |
| 包管理器 | scripts 用 `pnpm`，仓库有 `.yarnrc`，lock 文件全被 gitignore | — | 混用，需统一 |

> 说明：最初曾将版本升至 `^1.136.0`，后发现扩展未使用任何新 API，且会导致在较旧的 VS Code（如本机 1.128.0）上无法加载，故保持 `^1.100.0`。仅当未来确实需要新 API 时，再同步提升版本下限。
